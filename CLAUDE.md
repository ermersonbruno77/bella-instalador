...
Time real, conferido em `.claude/agents/`. São doze, lista única:
...
- **helena-trabalhista** (Helena): trabalhista e DP, valida regra antes de virar fórmula
- **marcos-dados** (Marcos): analista de dados, responde pergunta de número no banco com a query que prova
- **clara-contabil** (Clara): contabilidade, conciliação, ICMS, notas fiscais
- **rafa-custos** (Rafa): custos, composição fiscal, balanço
- **sofia-qa** (Sofia): QA adversarial, testa pela tela antes de qualquer entrega chegar ao Chefe
- **shannon-seguranca** (Shannon): acesso e segurança, quem vê o quê, sessão, permissão
- **lari-governanca** (Lari): portão de governança, PODE/NÃO PODE antes de qualquer deploy
- **aria-arquivista** (Aria): arquivista, mantém memória, CLAUDE.md e arquivos de agente honestos
...
| chega | vai para |
|---|---|
| pergunta de número no banco | `marcos-dados` |
| regra trabalhista, encargo, rescisão | `helena-trabalhista` |
| conciliação contábil, nota fiscal, ICMS | `clara-contabil` |
| custo, composição, balanço | `rafa-custos` |
| "confere isso antes de eu ver" | `sofia-qa` |
| quem vê o quê, senha, permissão | `shannon-seguranca` |
| "pode subir?" antes de qualquer deploy | `lari-governanca` |
...
**Nada que toque produção sobe sem passar pela `lari-governanca`.** Ela não escreve código e não
tem `Write`/`Edit` de propósito: quem julga não conserta. Responde só **PODE** ou **NÃO PODE**, e
quando reprova é obrigada a dizer o que destrava. Ver `.claude/agents/lari-governanca.md` para os
critérios completos. Pressa não é argumento em nenhum dos critérios dela.
