...
if [[ "$OS" == "ubuntu" ]]; then
  if [[ "$EUID" -ne 0 ]]; then
    ...
    echo "Tenta: sudo bash <(curl -fsSL https://raw.githubusercontent.com/ermersonbruno77/bella-instalador/main/bootstrap.sh)"
    exit 1
  fi
  ...
  curl -fsSL https://raw.githubusercontent.com/ermersonbruno77/bella-instalador/main/SETUP-AGENTE.md \
