...
aprendizado, portão de governança antes de deploy, checklist de pedido com vários itens). Nasceu
como clone sanitizado de uma instalação real, com todo dado sensível removido: nada de folha de
pagamento, nada de dado de terceiro, nada de informação pessoal.

O time vem com: `paulo-dev`, `juliana-ops`, `jonathan-copy`, `rafael-projetos`, `helena-trabalhista`,
`marcos-dados`, `clara-contabil`, `rafa-custos`, `sofia-qa`, `shannon-seguranca`, `lari-governanca`,
`aria-arquivista`. Ajuste ou remova o que não servir pro seu time antes de instalar: cada agente
