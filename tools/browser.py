#!/usr/bin/env python3
"""Navegador autonomo do agente (Playwright headless). Opera a web de verdade.

Uso:
  browser.py screenshot <url> [saida.png]     -> screenshot (full page); leia com Read (visao)
...
Sobre navegacao client-side (SPA): um clique que troca de rota (ex.: login
bem-sucedido, link interno) NAO dispara reload de pagina inteira. O <body> pode
ficar vazio por varios segundos entre uma rota e outra (o app faz checagem de
sessao e ate redirecionamentos encadeados antes de chegar na tela final). Depois
de um "click" que deve navegar, use "esperar_rota" com o padrao da URL de
destino em vez de "wait" com tempo fixo ou de forcar "goto" de novo (forcar
reload manual so mascara o problema e nao prova que a navegacao client-side
funciona). "wait" com ms fixo e para casos sem navegacao (ex.: esperar uma
animacao).
"""
...
# Sem isto, muitos sites de e-commerce respondem parede de antibot
# (tela de login, "clique para continuar", 403).
...
# Xvfb remove a maior parte disso na raiz; o resto e patch de fingerprint
# via init script.
    ...
    with sync_playwright() as p:
        ...
        try:
            for a in acoes:
                ...
                # Existe porque defeito de layout so se prova na largura em
                # que ele aparece.
                elif "viewport" in a:
                    v = a["viewport"]
